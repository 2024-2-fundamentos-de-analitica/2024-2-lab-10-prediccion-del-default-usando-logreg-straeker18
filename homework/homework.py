...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    """Limpia el dataframe: renombra columnas, elimina registros no válidos y ajusta categorías."""
    df_cleaned = df.copy()
...
def create_pipeline() -> Pipeline:
    """Crea un pipeline con preprocesamiento, selección de características y clasificación."""
...
def create_estimator(pipeline: Pipeline, x_train: pd.DataFrame) -> GridSearchCV:
    """Configura la búsqueda de hiperparámetros usando validación cruzada."""
    param_grid = {
...
def save_model(path: str, estimator: GridSearchCV):
    """Guarda el modelo comprimido en un archivo."""
    os.makedirs(os.path.dirname(path), exist_ok=True)
    with gzip.open(path, "wb") as f:
        pickle.dump(estimator, f)


def calculate_metrics(dataset_type: str, y_true: pd.Series, y_pred: pd.Series) -> Dict[str, Any]:
...
def calculate_confusion_matrix(dataset_type: str, y_true: pd.Series, y_pred: pd.Series) -> Dict[str, Any]:
...
def main():
    ...
    save_metrics([train_metrics, test_metrics, train_cm, test_cm], "files/output/metrics.json")
